...
FileInformationWiki <- FileInformationPrivatized

names(FileInformationWiki) <- c("Volume", "SynID", "File Name", "brightenid Freq", "userid Freq", "Observation Freq", "Mean Obs. Per userid", "Mean Obs. Per brightenid")
FileInformationWiki <- FileInformationWiki[,c(1,2,3,6,4,8,5,7)]
...
#generate description of overal volume of data for top of data information wiki subpages
{
  ...
  print(kable(FileInformationWiki, row.names = FALSE))
  cat("\n")
  sink()
}
...
unmatchedFiles <- c("v1_alchoholuse.csv","v1_apps.csv","v1_appssatisfaction.csv", "v1_mhealthscreen.csv", "v1_healthapps.csv", "v1_moodassessment.csv", "v2_gpsfeatures.csv")
namedUnmatched <- c("Alcohol Use", "Applications", "Satisfaction Survey", "Mental Health Screening", "Health Applications", "Mood Assessment", "GPS Features")
...
getwd()
wikiHolder$attachments <- as.list( images)
