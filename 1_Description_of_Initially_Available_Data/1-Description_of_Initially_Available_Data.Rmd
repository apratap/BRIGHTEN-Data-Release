...
namedMatched <- c("Passive Features","PHQ2","PHQ9","Study Metadata","QoL Impact","GAD7","Health App","General Mental Health","PGIC","Sleep")
...
The folowing chunk will attempt to update the project wiki with the available text files, suing the name of ecah file as the name for tha page in the wiki.
...
wikiHolder <- synapseclient$wiki$Wiki(title = "BRIGHTEN Study Wiki", owner = workingProject)
wikiHolder$markdown <- "This is automatically generated!"
...
for(i in 1:length(toUpload)) {
  ...
  wikiHolder <- synapseclient$wiki$Wiki(title = paste(i, " - ", toUpload[i], sep = ""), owner = workingProject, parentWikiId = wikiData, markdown = wikiMarkdown)
  SYNLOGIN$store(wikiHolder)
}
