...
for (i in 1:length(namedMatches)) {
  ...
  wiki1 <- data.frame("#" = 1:length(temp1),
                      "Variable Name" = names(temp1),
                      "Data Type" = sapply(temp1, class),
                      check.names = FALSE)
  
  wiki1$Range <- NA
  for( j in 1:nrow(wiki1)) {
    if (wiki1[j,]$"Data Type" == "factor") {
      if (length(levels(temp1[,j])) > 5 ) {
        x <- length(levels(temp1[,j]))
        wiki1[j,]$"Range" <- paste(as.character(levels(temp1[,j])[c(1,2,median(1:x),x-1,x)]), collapse = "\n ") 
      }
      if (length(levels(temp1[,j])) <= 5 ) {
        wiki1[j,]$"Range" <- paste(as.character(levels(temp1[,j])), collapse = ", ") 
      }
    }
    if (wiki1[j,]$"Data Type" != "factor") {
      wiki1[j,]$"Range" <- paste(as.character(range(temp1[,j], na.rm = TRUE)),sep =  " - ", collapse = " - ")
    }
  }
