...
simple.content.listing <- function(VOL, INFO) {
  ...
  #temp <- find.data(INFO$path)
                    
  for(i in 1:(length(INFO))) {
    extension <- tools::file_ext(INFO[[i]]$path)
...
ids <- c("user_id","brightenid","username","Response ID")
for (i in ids) {
  FileInformation[,i] <- 0
}
...
for (i in 1:nrow(FileInformation)) {
  extension <- tools::file_ext(FileInformation[i,]$path)
  if(extension == "tsv"| extension == "txt") {
    temp <-as.data.frame(fread(FileInformation[i,]$path, check.names = FALSE)) #read.table(INFO[[i]]$path, sep = "\t",  header = TRUE)
  }
  if(extension == "csv") {
    temp <- read.csv(FileInformation[i,]$path, header = TRUE, check.names = FALSE)
  }
  if(extension == "xls" | extension == "xlsx") {
    temp <- xlsx::read.xlsx(FileInformation[i,]$path, sheetIndex = 1)
  }  
  FileInformation[i,]$uniqueObservations <- nrow(unique(temp))
  for (j in ids) {
    if (FileInformation[i,j] == 1) {
      FileInformation[i,j] <- length(unique(temp[,j]))

    }

  }
}
#FileInformation[i,]$uniqueObservations <- 
