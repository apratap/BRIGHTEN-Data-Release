...
write.csv(FileInformation, paste(DATADIRECTORY, "LocalFileInformation.csv", sep = ""))
...
namedMatches <- c("Passive Features","PHQ2","PHQ9","Study Metadata","QoL Impact","GAD7","Health App","General Mental Health","PGIC","Sleep")

for (i in 1:length(namedMatches)) {
  print(namedMatches[i])
  ...
  #sink(paste(WIKI,namedMatches[i],".txt", sep = ""))
  for (j in c("temp1","temp2")) {
    print(paste("step",j, sep = " "))
    classes <- sapply(j,class)
    for (k in 1:length(classes)) {
      classes[k] <- classes[[k]][1]
      print(paste("stepA",k, sep = " "))
    }

    
    wiki <- data.frame("#" = 1:length(j),
                        "Variable Name" = names(j),
                        "Data Type" = unlist(classes),
                        check.names = FALSE)
    
  
    wiki$Range <- NA
    for( k in 1:nrow(wiki)) {
      print(paste("stepB",k, sep = " "))
      if (wiki[k,]$"Data Type" == "factor") {
        if (length(levels(j[,k])) > 5 ) {
          x <- length(levels(j[,k]))
          wiki[k,]$"Range" <- paste(as.character(levels(j[,k])[c(1,2,median(1:x),x-1,x)]), collapse = "\n ") 
        }
        if (length(levels(j[,k])) <= 5 ) {
          wiki[k,]$"Range" <- paste(as.character(levels(j[,k])), collapse = ", ") 
        }
      }
      if (wiki[k,]$"Data Type" != "factor") {
        wiki[k,]$"Range" <- paste(as.character(range(j[,k], na.rm = TRUE)),sep =  " - ", collapse = " - ")
      }    
    kable(wiki)
    }
