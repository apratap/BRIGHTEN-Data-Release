...
DATADIRECTORY <- "./Data/"
REPORTDIRECTORY <- "./Reports/1_"
WIKI <- paste(DATADIRECTORY,"WikiPages/", sep = "")
...
write.csv(FileContents, paste(REPORTDIRECTORY, "LocalOriginalFileContents.csv", sep = "")) 
...
MatchedRecords[nrow(MatchedRecords)+1,]<- add.match(FileContents, V1FileName =  "brighten_v1_study_metadata.tsv", V2FileName = "final_mdata_20180105172418-surveyexport_edited_apratap.txt")
...
MatchedRecords[nrow(MatchedRecords)+1,]<- add.match(FileContents, V1FileName =  "sleep_406.xls", V2FileName = "sleep_brighten_v2.tsv")

MatchedRecords[nrow(MatchedRecords)+1,] <- add.match(FileContents,  V1FileName = "qualityoflife_401.xls", V2FileName =  "sds_brighten_v2.tsv")
...
write.csv(MatchedRecords, paste(REPORTDIRECTORY, "LocalMatchedRecords.csv", sep = ""))   
...
# Generating Data Tables for Wiki

It has been requested to present initial data descriptions to the wiki. This code outputs text for copying and pasting into the Synapse Wiki. If possible automated routines for creating the wiki pages should be used instead?
...
#Building or Updating WIKI

The folowing chunk will attempt to rebuild the project wiki with the available text files, using the name of ecah file as the name for tha page in the wiki.
