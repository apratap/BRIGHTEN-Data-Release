...
clean.user.names <- function(DF) {
  ...
  if ("response_local" %in% names(DF)) {
    DF <- subset(DF, select = -response_local)
  }
  if ("response_utc" %in% names(DF)) {
    DF <- subset(DF, select = -response_utc)
  }
  if ("week" %in% names(DF)) {
...
V1 <- subset(V1, select = -c(baseline_phq9))
V2 <- subset(V2, select = -c(userAgent, time_started, onBoardingTime, sum_phq9, sum_phq10, Referer, heard_about_us, heard_about_us_language))  
...
V1 <- subset(V1, select = -c(start, phq2ResponseTimeSecs))
V2 <- subset(V2, select = -c(phq2_date, phq2_date_minus_one, phq2_hod))
...
#removing timing
V1 <- subset(V1, select = -c(start, phq9ResponseTimeSecs, sum_phq10, sum_phq9))
...
#Load
write.csv(V1 , paste(DATADIRECTORY, "New/V1_EnergyLevels.csv", sep = ""), row.names = FALSE)
...
names(V1) <- c("brightenid","userid","gad7_1","gad7_2","gad7_3","gad7_4","gad7_5","gad7_6","gad7_7","gad7_S")
...
#from v1 remove
#anything not aligned with V2, and sent time local and sent time utc
# keep response_time_utc and response time local
#move
#questions to wiki
...
names(V1)[4] <- "sds_2"
names(V1)[5] <- "sds_3"
names(V1)[6] <- "In the last week, how much were you set back by stressful events or personal problems such as work, home, social, health, or financial problems?"
names(V1)[7] <- "In the past week, how much support have you received from friends, relatives, co-workers, etc., as a percentage of the amount you needed to cope?"
V1$sds <- NULL
